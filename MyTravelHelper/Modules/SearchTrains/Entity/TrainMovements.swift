...
struct TrainMovement: Codable {
    ...
    init(from decoder: Decoder) throws {
		...
		if values.contains(.locationFullName) {
			locationName = try values.decodeIfPresent(String.self, forKey: .locationFullName) ?? ""
				} else {
					locationName = nil
				}
		
        let departure = try values.decode(String.self, forKey: .expDeparture)
        self.init(trainCode: trainCode, locationCode: locationCode, locationFullName: locationName ?? "",expDeparture: departure)
    }
}
